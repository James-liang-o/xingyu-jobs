...
<meta property="og:description" content="全国心智障碍可投岗位信息导航，帮助心智障碍青年实现就业。">
...
<style>
  ...
  /* 高对比度模式（body.hc） */
  body.hc{--text:#000;--sub:#1F2937;--bg:#fff;--card:#fff;--line:#000}
  body.hc .card{border:1px solid #000;box-shadow:none}
  body.hc a,body.hc .fm{text-decoration:underline}
  body.hc .badge,body.hc .tag{border:1px solid #000}
  body.hc .site-footer{color:#000}
...
.citytag{padding:10px 16px;background:#fff}
  ...
  .tag.mh-tag.on{background:#065F46;color:#fff;border-color:#065F46}
  /* 无障碍工具条 */
  .a11y{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}
  .a11y .ab{background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.35);color:#fff;font-size:14px;padding:9px 14px;border-radius:10px;cursor:pointer;line-height:1.5}
  .a11y .ab:hover{background:rgba(255,255,255,.28)}
  .a11y .ab.on{background:#fff;color:var(--primary);font-weight:600}
  /* 分享弹窗 */
...
<div class="header">
  ...
  <div class="a11y">
    <button class="ab" onclick="fontSize(-1)" title="缩小字号">A-</button>
    <button class="ab" onclick="fontSize(1)" title="放大字号">A+</button>
...
<div class="aboutpage" id="aboutPage">
  ...
  <div class="ap-card">
    <h3>这里有什么</h3>
    <ul>
      <li>全国 <span class="hl">__TOTAL__ 条</span> 助残岗位，按城市、职业、心智等级、有效/失效自由组合筛选</li>
...
function jobTagsHtml(j){
  var h = '';
  var mh = mhRange(j);
  if(mh) h += '<span class="tag mh-tag'+(selMh.indexOf(mh)>=0?' on':'')+'" data-mh="'+mh+'">心智障碍·'+mh+'可投</span>';
...
document.getElementById('filtersBar').addEventListener('click', function(e){
  ...
  while(t && t !== this){
    ...
    if(t.classList && t.classList.contains('f-clear')){
      ...
      curLetter = '全部'; shownMax = PAGE_SIZE; renderCities(); renderFilters(); ensureAll(function(){ render(); });
      return;
    }
    t = t.parentNode;
  }
...
function render(){
  ...
  JOBS.forEach(function(j){
    ...
    if(selCities.length){
      if(selCities.indexOf(j.c) < 0) return;
...
// ===== 无障碍辅助：字号 / 高对比 / 深色 / 朗读 =====
function setFont(v){ try{ document.documentElement.style.fontSize = v + 'px'; localStorage.setItem('xy_fs', v); }catch(e){} }
function fontSize(d){
  var cur = parseInt(document.documentElement.style.fontSize || '16', 10);
  var v = Math.min(24, Math.max(14, cur + d * 2));
  setFont(v);
  toast('字号已设为 ' + v + 'px' + (v >= 20 ? '（最大）' : ''));
}
(function(){ try{ var v = parseInt(localStorage.getItem('xy_fs'), 10); if(v) setFont(v); }catch(e){} })();
function toggleHC(){
  var on = document.body.classList.toggle('hc');
  document.getElementById('hcBtn').classList.toggle('on', on);
  try{ localStorage.setItem('xy_hc', on ? '1' : ''); }catch(e){}
  toast(on ? '已开启高对比度模式' : '已关闭高对比度模式');
}
(function(){ try{ if(localStorage.getItem('xy_hc') === '1'){ document.body.classList.add('hc'); document.getElementById('hcBtn').classList.add('on'); } }catch(e){} })();
function toggleDark(){
...
// ===== 不蒜子访问统计（主域名失败自动切备用域名） =====
(function(){
  var domains = ['https://busuanzi.icp.gov.cn/js/busuanzi.pure.mini.js', 'https://busuanzi.ibruce.info/busuanzi/2.3/busuanzi.pure.mini.js'];
  var i = 0;
  function load(){
    if(i >= domains.length) return;
    var s = document.createElement('script');
    s.src = domains[i++]; s.async = true;
    s.onerror = load;
